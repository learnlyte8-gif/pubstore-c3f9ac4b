                    ...
                    DataRow(cells: [
                      ...
                      for (final p in products)
                        DataCell(SizedBox(
                          width: 120,
                          height: 100,
                          child: p.cover.isEmpty ? const Icon(LucideIcons.image, color: AppColors.muted) : CachedNetworkImage(imageUrl: p.cover, fit: BoxFit.contain),
                        )),
                    ]),
                    _row('Price', products.map((p) => '\$${p.price.toStringAsFixed(2)}').toList()),
                    _row('Rating', products.map((p) => '★ ${p.rating.toStringAsFixed(1)}').toList()),
                    _row('Sold', products.map((p) => '${p.soldCount}').toList()),
                    _row('Category', products.map((p) => p.categorySlug ?? '').toList()),
