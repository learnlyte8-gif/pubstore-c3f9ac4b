...
BEGIN
  ...
  SELECT COALESCE(round(sum(c.commission_amount), 2), 0) INTO promo_amt
  FROM public.commissions c
  WHERE c.order_id = o.id
    AND c.status NOT IN ('cancelled', 'reversed');

  promo_amt := LEAST(GREATEST(promo_amt, 0), GREATEST(goods_amt - commission_amt, 0));
  net_amt := GREATEST(goods_amt - commission_amt - promo_amt, 0);

  -- Only move money when there is something to move; a zero payout must not
  -- abort settlement (apply_wallet_transaction rejects zero amounts).
  IF net_amt > 0 THEN
    PERFORM public.apply_wallet_transaction(
      ...
      'Sale ' || COALESCE(o.ref_code, o.id::text)
        || CASE WHEN commission_amt > 0
                ...
                ELSE '' END
        || CASE WHEN promo_amt > 0
                THEN CASE WHEN commission_amt > 0 THEN ' + ' ELSE ' (net of ' END
                     || '$' || to_char(promo_amt, 'FM999990.00') || ' promoter commission'
                ELSE '' END
        || CASE WHEN commission_amt > 0 OR promo_amt > 0 THEN ')' ELSE '' END,
      o.id::text, 'sales');
  END IF;
